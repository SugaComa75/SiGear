...
app.post('/v1/mint', (req, res) => {
  ...
  // Basic sandbox rules: require proofing_tier and public_key_jwk
  if (!body.proofing_tier || !body.public_key_jwk) {
    return res.status(400).json({ error: 'proofing_tier and public_key_jwk are required' });
  }

  // If request appears to be production (host header), we simulate stricter checks by rejecting missing auth
  // For a local mock this is permissive; presence of Authorization header is enough for sandbox.
  if (!auth) {
    // Allow running locally without auth but warn in response
    // continue
  }
...
app.post('/v1/mint/:nti_id/reproof', (req, res) => {
  ...
  // In this mock, accept reproof and update the tier
